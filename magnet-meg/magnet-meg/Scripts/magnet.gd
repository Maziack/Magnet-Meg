...
@export var orbitRadius:float:													#in meters	100pix = ~3m
	set(newOrbitRadius):
		orbitRadius = newOrbitRadius
		if $Orbit/CollisionShape2D:
			$Orbit/CollisionShape2D.shape.radius = newOrbitRadius*px_to_m
...
func player_orbit():
	...
	for body in bodies:
		if body.name == "Player":
			if player.is_magnetized == 1:
				player.gravity_component.gravity = 0
			...
			targetPos = orbitPos+(0.75* targetVector * orbit.get_node("CollisionShape2D").shape.radius)
		...
		else: player.gravity_component.gravity = player.originalGravity
